...
```{r load_data, eval=TRUE,message=F,warning=F,results='asis',echo=TRUE}
...
```{r GetResultsFromJunctionSeq,eval=F,echo=TRUE,cache=TRUE}
...
in GOSJ R package:
```{r makeGeneWiseTable,eval=F,echo=TRUE,cache=TRUE}
...
```{r output_DE_genes_from_feature,eval=F,message=F,warning=F,results='asis',echo=F,cache=F}
head(Re.pwf.exon.sj)
dim(Re.pwf.exon.sj)
...
```{r genewiseFDR,eval=F,message=F,warning=F,results='asis',echo=F,cache=F}
...
```{r DE_gene_feature,eval=F,message=F,warning=F,results='asis',echo=F,cache=F}
...
```{r DE_gene_feature_new_cutoff, eval=F,message=F,warning=F,results='asis',echo=F,cache=F}
...
```{r DE_gene_feature_GO_BP_only, eval=F,message=F,warning=F,results='asis',echo=F,cache=F}
...
```{r DE_gene_feature_geneWise_rMAT,eval=F,message=F,warning=F,results='asis',echo=F,cache=F}
...
```{r DE_gene_feature_geneWise_rMAT_2_fold_change,eval=F,message=F,warning=F,results='asis',echo=TRUE,cache=TRUE}
...
```{r DE_from_PJ,eval=TRUE,message=F,warning=F,results='asis',echo=TRUE,cache=F}
...
```{r DE_from_PJ_files_we_got,eval=TRUE,message=F,warning=F,results='asis',echo=TRUE,cache=F}
...
#Generate DE list 
```{r re.rMAT,eval=TRUE,message=F,warning=F,results='asis',echo=TRUE,cache=TRUE}
#load("/media/H_driver/PJ/JunctionRe.RData")
qDe_defined_by_what="P_and_inclusion"
re.rMAT<-ProcessOutputFilesFrom_rMATS(dir.name.rMAT,input.file.pattern,De_defined_by_what)

Re.PJ.selected.feature.2.FC.p<-Select_DE_gene_basd_on_Feature(Re.PJ,re.PJ.gene.based,re.rMAT,"SE",1,0.05,output.dir.DE.name)
...
* red: DE defined the genewise FDR of each gene derived from p values of all features within gene from JunctionSeq  
* orange:DE defined by Fold Change(FC) and p value features(exons and splicing junctions)(FC>2 and p value <0.05)
* blue:  DE defined by p value and IncLevelDifference from rMAT(p value <0.05 and IncLevelDifference>0.2)
...
```{r JunctionSeq_rMAT_GO_output,eval=TRUE,message=F,warning=F,results='asis',echo=T,cache=F}
output.dir.name="/media/H_driver/PJ/Results/"
output.dir.name.rMAT_SE_based=paste0(output.dir.name,"test_7_25_2016","_","Feature_GeneWise_rMAT_SE_based/")
dir.create(output.dir.name.rMAT_SE_based)
...
```{r DE_JunctionSeq_p_FC_vs_SJ,eval=F,message=F,warning=F,results='asis',echo=F,cache=F}
...
```{r JunctionSeq_DE_FDR_vs_SJ,eval=F,message=F,warning=F,results='asis',echo=F,cache=F}
...
```{r rMAT_DE_SJ,eval=TRUE,message=F,warning=F,results='asis',echo=F,cache=F}
...
```{r save_all,eval=TRUE,message=F,warning=F,results='asis',echo=F,cache=F}
