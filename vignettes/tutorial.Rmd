...
pandoc_args: --natbib
biblio-style: plain
vignette: "%\\VignetteIndexEntry{PathwaySplice}  %\\usepackage[utf8]{inputenc} %\\VignetteEngine{knitr::rmarkdown}\n
  \n"
---
...
Note that typically selection bias remains when "fdr" instead of "pvalue" (default option) is used to select significant genes in `makeGeneTable`, becasue FDR has a monotonic relationship with p-values. For example, in our `featureBasedData`, if we had selected significant genes using "fdr" option, the boxplot below shows substantial selection bias remains. 

```{r eval=TRUE, message=FALSE, warning=FALSE, results='markup'}
gene.based.table <- makeGeneTable(featureBasedData, stat = "fdr")
...
Note that the default option for `use.genes.without.cat` was set to `FALSE`, this means that genes not mapped to any tested categories are ignored in analysis. In order for all genes in `genewise.table` to be counted towards the total number of genes outside the category (i.e. genes in the background), this option should be set to `TRUE`.  
...
Note that the input of `enrichmentMap` function is an object returned from `runPathwaySplice` function, so `enrichmentMap` function supports both GO annotation as well as customized genesets. 
...
In **runPathwaySplice** function, the choices for argument `method` include 

* `Hypergeometric` which does not adjust for bias
* `Wallenius` which adjust bias based on the  Wallenius  non-central  hypergeometric  distribution
* `Sampling` which uses random sampling to approximate the true distribution. It takes `gene.based.table` as input and then calculates weights for each gene using a spline regression model, which is the probability a gene being significant given the number of gene features (e.g. exons) associated with it. Next, random subsets of genes are sampled from the experiment. Each sub?set contains the same number of genes as the observed number of significant genes. For genes in each subset, the probability of selecting a gene is equal to the weights calculated above, and the number of genes belonging to a GO category is then calculated. Many random subsets of genes are generated to produce the null distribution for estimating p-value of a GO category.

The figure below plots pathway analysis p-values for the MDS dataset obtained using Hypergeometric test (Hyper), Wallenius approximation (Wall), 30k random sampling (Sampling_30k) and 200k random sampling (Sampling_200k) vs. p-values based on 200k random sampling (Sampling_200k). This was accomplished using the **compareResults2** function
...
In the figure above, each dot represents a GO category. The results show that GO category p-values for 30k random sampling and 200k random sampling are virtually indistinguishable (blue crosses and red triangles are almost exactly the same), p-values based on Wallenius approximation are also highly congruent with sampling results. On the other hand, unadjusted results using Hypergenometric distribution are markedly different from both sampling and Wallenius p-values. In terms of time, 200k random sampling took more than 1 hour, while Wallenius approximation took less than a minute. Therefore, we recommend using Wallenius approximation (default for **runPathwaySplice** function), unless the number of gene sets tested is small (e.g. less than 100 gene sets).    
